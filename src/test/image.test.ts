...
describe('ImageRoute', () => {
  ...
  describe('POST /image/multiple', () => {
    it('should return 400 if no files were uploaded', async () => {
      const res = await request(app.getServer()).post('/image/multiple').expect(400);

      expect(res.text).toBe('No files were uploaded');
    });

    it('should return 400 if uploaded file is not an image', async () => {
      const res = await request(app.getServer()).post('/image/multiple').attach('image', './test/data/not-an-image.txt').expect(400);
