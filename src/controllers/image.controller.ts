...
export class ImageController extends BaseController {
  ...
  public async getImage(req: Request, res: Response) {
    ...
    return res.set('Content-Type', contentType).sendFile(path.join(__dirname, `../../${FILE_IMAGE_PATH + filename}`));
  }
  ...
  public async resizeGIF(filename: string, originalFilePath: string): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      execFile(gifsicle, ['--resize-fit-width', '300', '-o', outputFilePath, originalFilePath], err => {
        if (err) {
          reject(err);
        }
        resolve();
  ...
  public async resizedImage(originalFilePath: string, newFilePath: string, maxSize: number): Promise<void> {
    ...
    const imageInfo = imageSize(originalFilePath);
    ...
    await sharp(originalFilePath).resize(newWidth, newHeight).toFile(newFilePath);
